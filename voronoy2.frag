...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 point1 = vec2(0.3 + noise(vec2(iTime, 0.1)), noise(vec2(iTime, 1.4)));
    vec2 point2 = vec2(0.3 + noise(vec2(iTime, 2.2)), noise(vec2(iTime, 3.5)));
    vec2 point3 = vec2(0.3 + noise(vec2(iTime, 4.3)), noise(vec2(iTime, 5.6)));
    ...
    float minD = 2.0;
    float vd1 = distance(uv, point1);
    if (vd1 < minD) {
        color = vec3(1. - vd1, 0.,0.);
        minD = vd1;
    }
    float vd2 = distance(uv, point2);
    if (vd2 < minD) {
        color = vec3(0., 1. - vd2, 0.);
        minD = vd2;
    }

    float vd3 = distance(uv, point3);
    if (vd3 < minD) {
        color = vec3(0., 0., 1. - vd3);
        minD = vd3;
    }
    ...
    // color = vec3(minD);
    // color -= sin(150. * minD) * minD;
    // color -= step(.7, sin(iTime * 3. * minD));
