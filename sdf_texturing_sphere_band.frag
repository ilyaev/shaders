...
#iChannel1 "file://st_texture_cats.png"
#iChannel2 "file://st_texture_disp_1.png"
...
float GetDist(vec3 p) {
    ...
    vec2 uv = vec2(atan(p.x, p.z)/6.2832 + .5, .7*p.y/3. + .5);
    float disp = texture(iChannel2, uv).r;
    disp *= smoothstep(1.4, 1., abs(p.y));
    // disp += sin(p.z*p.y * 5.) * 0.82;
    d -= disp * .2;
    return d;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    if(d<MAX_DIST) {
        ...
        uv = vec2(atan(p.x, p.z)/6.2832 + .5, 2. * p.y/3. + .5);

        uv.x -= iTime *.1;
        uv.y -= sin(iTime*10.) * .06;

        vec4 cats = texture(iChannel1, uv);

        cats.a *= smoothstep(0.6, 0.5, abs(p.y)); // hide poles
