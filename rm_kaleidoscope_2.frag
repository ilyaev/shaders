...
vec3 renderSegment(vec2 uv, float id) {
    float n = n21(vec2(id, 1.));
    vec3 col = vec3(0.);
    uv -= vec2(1.,.4);
    float d = abs(length(uv));

    // col += step(.1, d);
    // col += .1/pow(d, 1.2) * vec3(.9, .3, .1);

    float box = sdBox(uv, vec2(0.), vec2(.38, .1));
    if (id != 0.) {
        col += step(0., box);
    } else {
        col = vec3(1.);
...
float getCorridor(vec3 q) {
    ...
    // p.xy *= rot2d(iTime);
    p = kaleidoscopeUV(p, 8.);


    vec3 pos1 = vec3(1., 0., 0.);

    float box = sdBox(p - pos1, vec3(.1, .6, .5));
...
float getK(vec3 q) {
    ...
    return min(d3, min(d2, d));//min(d, d1));
}
