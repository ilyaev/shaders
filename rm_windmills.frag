...
float getHeight(vec2 p) {
    // return 0.;
    float h = noise(p/3. - iTime*0.)*.3 + noise(p/2. + iTime*2.*0.)*.5;
    return h;
}
...
vec3 getAlbedoByMaterial(float material, vec3 p, vec3 rd, vec3 normal) {
    ...
    if (material == 1.) {
        ...
        // float a = atan(p.x, p.y) + PI/8.;
        // float segmentAngle = PI2 / 8.;
        // float wid = floor((a + PI) / segmentAngle);
        // if (wid == 8.) {
        //     wid = 0.;
        // }
        // a -= segmentAngle * floor(a / segmentAngle);

        // vec2 uv = vec2(abs(p.z - .5), a);
        ...
        albedo *= vec3(.1, .5, .0);
        // step(0.0001, sin(p.x*size)*sin(p.z*size)) *
    }
    return albedo;
}
...
float getMill(vec3 q, vec2 id) {
    ...
    p = kaleidoscopeUV(vec3((p.xy - vec2(0., height)) * rot2d(iTime * n*10.), p.z), 3.);
...
float getMills(vec3 q) {
    vec3 p = q - vec3(0., 0., -7.);

    float d = 10000.;

    vec3 bounds = vec3(2., 0., 2.);

    vec3 baseSpacing = vec3(5.0,0., 10.);

    vec3 l = bounds;
    vec3 rc1 = vec3(baseSpacing);

    vec3 fid = round(p/rc1);
    vec2 id = fid.xz;

    p = p - rc1 * clamp(fid, -l, l);
    ...
    vec3 p1 = p - vec3(0., n*3. - 1.5, 0.);
    p1.xz *= rot2d(PI/2. * n - PI/4.);
    d = getMill(p1, id);
...
vec3 getDist(vec3 p) {
    ...
    float plane = sdPlane(p - vec3(0., -3. + height, 0.), .1);
    float dK = getMills(p);
    float d = dK;
    d = min(d, plane);
    if (d == dK) {
...
void mainImage(out vec4 fragColor, in vec2 fragCoords) {
    ...
    vec3 lightPos = vec3(2.1, 1.7, -6.5);
