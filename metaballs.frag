...
vec3 getStar(vec2 center, vec2 cell, float radius, int i) {
    float d = distance(cell, center);
    ...
    vec3 color = vec3(r/pow(d,1.5 + float(10 - i)/10.), r/pow(d,2.), r/pow(d,3.5));
    return color;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    float shift = CELLS / 12. ;//+ 3. * sin(iTime*10.);

    float start = -5. * shift;

    for(int i = 0 ; i <= 10 ; ++i) {
        // vec2 c = vec2((shift + cos(iTime+float(i))*shift) * float(i), CENTER.y + shift*0.75*sin(iTime*3. + PI/8.*float(i))*3.); // one
        vec2 c = vec2(CENTER.x, CENTER.y);
        float metaWave = 0.;//cell.y/1024.*cell.x;
        c += vec2(start + float(i) * shift, sin(iTime*10. + metaWave + float(i)*PI/8.)*starSize); // worm
        color += getStar(c, cell, starSize, i);
    }
